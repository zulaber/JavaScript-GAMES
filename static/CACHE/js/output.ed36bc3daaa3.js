...
function check_math(){
	...
	if (chosen_cards[0] === chosen_cards[1]){
		...
		alert(win_cards.includes(chosen_cards[0]))
		if (win_cards.includes(chosen_cards[0])){
			alert('fsd')
		}
		alert('Yup, correct')
		win_cards.push(chosen_cards.name[0])
		win_cards.push(chosen_cards.name[1])
	} else {
	...
	chosen_cards = []
	chosen_id = []
	let uniq = win_cards => [...new Set(win_cards)];
	alert(win_cards)
	alert(typeof(win_cards))
